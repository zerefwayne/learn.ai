...
class ModelCreator {
  ...
  addConvLayer(n_filters, kernelSize = 3,activation = "relu") {
    if (this.previousInputShape != -1) {
      this.model.add(
        tf.layers.conv2d({
          filters: n_filters,
...
function initializeModelFromJSON(modelJson) {
  ...
  layers.forEach(layer => {
    if (layer.type === "dense") {
      ...
      creator.addConvLayer(layer.n_filters, layer.kernel_size, layer.activation)
    } else if (layer.type === "maxpool") {
      creator.addMaxPoolLayer(layer.kernel_size)
    }
  });
